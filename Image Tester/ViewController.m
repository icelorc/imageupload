...
- (IBAction)sendPicture:(id)sender {
    ...
    NSURL *url = [NSURL URLWithString:@"http://testjson.herokuapp.com/products.json"];
...
- (void)sendImagesToServer:(NSURL *)url
{
    ...
    UIImage *testImage = [UIImage imageNamed:@"01.jpg"];
    
    NSData *imageData = UIImageJPEGRepresentation(testImage, 1.0);
    [Base64 initialize];
    NSString *imageDataEncodedString = [Base64 encode:imageData];
    [selectedImages addObject:[NSDictionary dictionaryWithObject:imageDataEncodedString forKey:@"avatar_data"]];
    ...
    NSMutableDictionary *newProject = [[NSMutableDictionary alloc] initWithObjects:[NSArray arrayWithObjects:@"邵祥", @"邵祥好棒", imageDataEncodedString, nil] forKeys:[NSArray arrayWithObjects:@"name", @"description", @"image", nil]];
